...
//-----------------------------------------------------------------------------
/*:
 * @plugindesc 用于显示事件上的浮动图标(经过修改，不具备通用性)
 * @ver v.1.0
 * @url http://galvs-scripts.com
 * @target MZ
 * @author Galv & Blacktunes
 *
 * @param yOffset
 * @name Y Offset
...
Galv.AI.setMoveTip = function (event) {
...
Sprite_ActionIcon.prototype.initialize = function () {
	...
	$gamePlayer.actionIconTarget = { ...$gamePlayer.actionIconTarget, ...{ eventId: -1, iconId: 0, xy: { x: 0, y: 0 } } };
	this._iconIndex = 0;
	this.z = Galv.AI.z;
	this.changeBitmap();
...
Sprite_ActionIcon.prototype.update = function () {
	...
	this.x = ($gamePlayer.actionIconTarget.eventId === 0 ? $gamePlayer : $gameMap.event($gamePlayer.actionIconTarget.eventId)).screenX() + this._offsetX + commentX;
	this.y = ($gamePlayer.actionIconTarget.eventId === 0 ? $gamePlayer : $gameMap.event($gamePlayer.actionIconTarget.eventId)).screenY() + this._offsetY + this._float + commentY;
